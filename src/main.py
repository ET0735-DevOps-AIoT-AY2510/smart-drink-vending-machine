...
def process_payment():
    ...
    if drink and (drink['stock_quantity'] - drink['reserved_stock']) > 0:
        ...
        try:
            conn.execute('UPDATE Drinks SET reserved_stock = reserved_stock + 1 WHERE drink_id = ?', (drink_id,))
...
def collect_drink(order_id):
    ...
    try:
        ...
        order = conn.execute('SELECT drink_id FROM Orders WHERE order_id = ?', (order_id,)).fetchone()
        if order:
            drink_id = order['drink_id']
            # Decrease stock_quantity and reserved_stock
            conn.execute('UPDATE Drinks SET stock_quantity = stock_quantity - 1, reserved_stock = reserved_stock - 1 WHERE drink_id = ?', (drink_id,))
            # Update order status
